...
@File    :   data_prpcessor_seq.py
 
@Time    :   2020-01-10 17:56
...
if __name__ == "__main__":
    vocab_file = "/Data/public/Bert/chinese_wwm_L-12_H-768_A-12/vocab.txt"
    tokenizer = tokenization.FullTokenizer(vocab_file=vocab_file)
    label2id = json.loads(open("label2id.json").read())

    max_seq_len = 64

    prepare_tf_record_data(tokenizer, max_seq_len, label2id, path= basedir + "/data/train.json",
                           out_path="data/train.tf_record")
    prepare_tf_record_data(tokenizer, max_seq_len, label2id, path= basedir + "/data/dev.json",
